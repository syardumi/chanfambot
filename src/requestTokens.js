...
class RequestTokens {
  ...
  parseMessageComponents () {
    ...
    parts.forEach((part) => {
      ...
      if (!this.username || ((this.operation.includes('transfer') || this.operation.includes('move')) && !this.recipient)) {
  ...
  async runMessageCommand () {
    let anyoneCmdFound = true, modCmdFound = true
    ...
    if (!this.isSuperUser) {
      if (anyoneCmdFound) {
        ...
        this.client.say(this.target, `Command not found or you are not a mod. Example: !token {operation} {@user} {# of tokens} {song title}`)
    ...
    if (this.chatMsg.replace(' ', '').includes('!tokenhelp') || this.chatMsg.replace(' ', '').includes('!tokenshelp')) {
      this.client.say(this.target, `Example: !token {operation} {@user} {# of tokens} {song title}`);
    }

    // - !token operation help :: shows operations
    else if (this.chatMsg.includes('!token operation help') || this.chatMsg.includes('!tokens operation help')) {
      this.client.say(this.target, `Operations: check, [add, give], [take, sub, subtract, remove, rm, livelearn, llearn, ll, songbump, bump], [transfer, move], clear, history, modhistory`);
    }

    // - !token (check) {@user} :: check tokens for the param user
    else if (this.chatMsg !== '!token' && this.chatMsg !== '!tokens' && this.operation === 'check' && this.username) {
      await this.tokenCheck(this.username)
    }

    // - !token add {@user} {# of tokens} :: add tokens to user
    else if ((this.operation === 'add' || this.operation === 'give') && this.username && this.numOfTokens) {
      await this.tokenAdd()
    }
    ...
    if (anyoneCmdFound || modCmdFound) {
      ...
      this.client.say(this.target, `Command not found or is missing parameters. Example: !token {operation} {@user} {# of tokens} {song title}`)
      console.log(`* No match: ${this.chatMsg} command`);
    }
  }
  ...
  async tokenSubtract () {
    if (this.operation === 'll' || this.operation === 'llearn' || this.operation === 'livelearn') {
  ...
  async tokenHistory () {
    const result = await this.db.all('SELECT * FROM request_token_history WHERE username = ? AND channel = ? AND is_cleared = 0 ORDER BY timestamp DESC LIMIT 5', this.username, this.channel)
  ...
  async tokenModHistory () {
    const result = await this.db.all('SELECT * FROM request_token_history WHERE mod_username = ? AND channel = ? AND is_cleared = 0 ORDER BY timestamp DESC LIMIT 5', this.username, this.channel)
    let msg = `No token history for ${this.username}`
    if (result && result.length) {
